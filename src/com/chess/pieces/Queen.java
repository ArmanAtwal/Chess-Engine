...
/**
 * In this class, we are simply designing the queen piece, and its legal movements
 * based on a variety fo factors such as piece position, alliance, etc. (validity as well).
 * It's simply a combination of the rook and bishop classes!
 */
public class Queen extends Piece{

    private final static int[] MOVE_COORDINATES_VECTOR = {-9, -8, -7, -1, 1, 7, 8, 9};
    // These are all the possible amount of spaces on a board,
    // that a queen could move, in a perfect world
     ...
     * move
     */
    public Queen(int piecePosition, Color pieceColor, final boolean isFirstMove) {
        super(PieceType.QUEEN, piecePosition, pieceColor, isFirstMove);
    }

    /**
     * The goal of this method is for us to apply all possible moves of a queen to a queen at a current position,
     ...
     * @return new Queen(action.getDestinationTile(), action.getMovedPiece().getPieceColor());
     * This returns a new object amongst the piece subclasses, and it passes in the new position along with
     * the color
     */
    @Override
    public Queen movePiece(Action action) {
        return new Queen(action.getDestinationTile(), action.getMovedPiece().getPieceColor());
    }

    /**
     * When this piece needs to be printed as part of our board, we will use the enumerator, and specify queen, so it
     * prints "Q"
     ...
     * @return BoardUtils.FIRST_COLUMN[currentPosition] && ((futureOffset == -1 || futureOffset == -9) || futureOffset == 7);
     ...
     * the first column, and our offset is an exception, of -9, 7, or -1 then we have reached a futureOffset we can't jump to.
     */
     ...
     * @param currentPosition is the pieces current board placement
     * @param futureOffset is the pieces future board placement
     * @return BoardUtils.EIGHTH_COLUMN[currentPosition] && ((futureOffset == -7) || futureOffset == 1 || futureOffset == 9);
     * What this basically means is we have a boolean array as part of BoardUtils. Inside this boolean array,
     * everything in the eighth column is true, else is false. If our current position happens to fall inside
     * the eighth column, and our offset is an exception, of -7, 9, or 1 then we have reached a futureOffset we can't jump to.
     */
